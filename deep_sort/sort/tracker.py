...
class Tracker:
    ...
    def __init__(self, metric, max_iou_distance=0.9, max_age=15, n_init=3, _lambda=0): #max_age=30
        self.metric = metric
    ...
    def update(self, detections, classes, hc_boxes_norm):
        ...
        for track_idx, detection_idx in matches:
            self.tracks[track_idx].update(
                self.kf, detections[detection_idx], classes[detection_idx], hc_boxes_norm[detection_idx])
        for track_idx in unmatched_tracks:
            self.tracks[track_idx].mark_missed()
        for detection_idx in unmatched_detections:
            self._initiate_track(detections[detection_idx], classes[detection_idx].item(), hc_boxes_norm[detection_idx])
        self.tracks = [t for t in self.tracks if not t.is_deleted()]
        
        # TODO - Modify Code
        self.tracks_deleted = [t for t in self.tracks if t.is_deleted()]
        self.tracks_tentative = [t.track_id for t in self.tracks if not t.is_confirmed()] 
    ...
    def _initiate_track(self, detection, class_id, org_input_box):
        mean, covariance = self.kf.initiate(detection.to_xyah())
        self.tracks.append(Track(
            mean, covariance, self._next_id, class_id, self.n_init, self.max_age,
            detection.feature, detection, org_input_box))
        self._next_id += 1
